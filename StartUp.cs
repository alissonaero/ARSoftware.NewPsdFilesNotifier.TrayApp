...
using ARSoftware.NewPsdFilesNotifier.TrayApp.Common;
using ARSoftware.NewPsdFilesNotifier.TrayApp.Models;
using ARSoftware.NewPsdFilesNotifier.TrayApp.Properties;
...
namespace ARSoftware.NewPsdFilesNotifier.TrayApp
{
		...
		private void PhotoShopClosingWhatcher(string[] files)
		{
			try
			{
				...
				foreach (var file in files)
				{
					...
					{
						MessageBoxHelper.ShowError($"Não foi possível mover os arquivos para a pasta de arquivos processados.{Environment.NewLine}{IOEx.Message}");
					}
					catch (Exception ex)
					{
						MessageBoxHelper.ShowError($"Não foi possível mover os arquivos para a pasta de arquivos processados.{Environment.NewLine}{ex.Message}");
					}
				}
